...
exports.create_student = function (req, res) {
    if (req.method == "POST") {
        ...
        var sql = "INSERT INTO student_account(rollnumber, username, password, name, class) VALUES ('" + rollnumber + "','" + username + "','" + hash + "','" + name + "','" + classname + "')"
...
exports.edit_student = function (req, res) {
    if (req.method == "POST") {
        ...
        var hash = crypto.createHash('md5').update(password).digest("hex") 
        var sql = "UPDATE student_account SET rollnumber='" + rollnumber + "',username='" + username + "',password='" + hash + "',name='" + name + "',class='" + classname + "',contest_id='" + contest_id + "',ip='" + ip + "',timeout='" + formatTime(timeout) + "',islogin='" + islogin + "' WHERE userId='" + id + "'"
        db.query(sql, function (err) {
            if (err) {
                res.redirect("/admin/student")
            } else {
...
exports.create_teacher= function (req, res) {
    if (req.method == "POST") {
        ...
        var password = post.password
        var hash = crypto.createHash('md5').update(password).digest("hex") 
        var sql = "INSERT INTO teacher_account(rollnumber, username, password, name) VALUES ('" + rollnumber + "','" + username + "','" + hash + "','" + name + "')"
        db.query(sql, function (err) {
            if (err) {
                req.session.sql_err = true
